...
# Apply a theme or custom style
root.configure(bg='#22c1c3')  # Light gray background

# Create a central frame for layout with a stylish border
central_frame = tk.Frame(root, padx=20, pady=20, bg='#ffffff', relief='groove', borderwidth=2)
central_frame.place(relx=0.5, rely=0.5, anchor='center')

# Add Salary Dashboard heading
header_label = tk.Label(root, text="Salary Dashboard", bg='#22c1c3', font=('Arial', 16, 'bold'))
header_label.place(relx=0.5, rely=0.2, anchor='center')

# Grid configuration for the central frame
central_frame.grid_columnconfigure([0, 1], weight=1)
central_frame.grid_rowconfigure([0, 1, 2, 3, 4, 5], weight=1)

# Create and place widgets with enhanced styling
tk.Label(central_frame, text="Age:", bg='#ffffff', font=('Arial', 12, 'bold')).grid(row=0, column=0, padx=10, pady=10, sticky="e")
age_entry = tk.Entry(central_frame, font=('Arial', 12))
age_entry.grid(row=0, column=1, padx=10, pady=10, sticky="w")

tk.Label(central_frame, text="Years of Experience:", bg='#ffffff', font=('Arial', 12, 'bold')).grid(row=1, column=0, padx=10, pady=10, sticky="e")
experience_entry = tk.Entry(central_frame, font=('Arial', 12))
experience_entry.grid(row=1, column=1, padx=10, pady=10, sticky="w")

tk.Label(central_frame, text="Current Job Title:", bg='#ffffff', font=('Arial', 12, 'bold')).grid(row=2, column=0, padx=10, pady=10, sticky="e")
current_job_title_combobox = ttk.Combobox(central_frame, state="readonly", font=('Arial', 12))
current_job_title_combobox.grid(row=2, column=1, padx=10, pady=10, sticky="w")

tk.Label(central_frame, text="New Job Title:", bg='#ffffff', font=('Arial', 12, 'bold')).grid(row=3, column=0, padx=10, pady=10, sticky="e")
new_job_title_combobox = ttk.Combobox(central_frame, state="readonly", font=('Arial', 12))
new_job_title_combobox.grid(row=3, column=1, padx=10, pady=10, sticky="w")

predict_button = tk.Button(central_frame, text="Predict Salary", command=predict_salary, font=('Arial', 12, 'bold'), bg='#4CAF50', fg='#ffffff', relief='raised')
predict_button.grid(row=4, column=1, padx=10, pady=10, sticky="w")

result_label = tk.Label(central_frame, text="", bg='#ffffff', font=('Arial', 12, 'bold'))
...
current_job_title_combobox['values'] = job_titles
new_job_title_combobox['values'] = job_titles
